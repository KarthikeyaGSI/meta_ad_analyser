  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (sessionData?.data?.user) {
        useStore.getState().setUser(sessionData.data.user as any);
      }

      setStatus('success');
      setTimeout(() => {
        // Redirect to dashboard (layout will redirect to onboarding if onboardingCompleted is false)
      ...
      <div className="w-full max-w-[400px]">
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-500 text-sm mt-2">
            {step === 'email' ? 'Get started with Vero to automate your meta ads' : 'Enter the 6-digit code sent to your email'}
          </p>
        </div>
